...
def find_admin_panels(target_url, paths, num_threads):
    ...
    def divide_and_scan(paths, num_threads):
        ...
        with tqdm(total=total_paths, desc="Scanning Admin Panels", ncols=100, bar_format="{l_bar}{bar} {n_fmt}/{total_fmt}") as progress_bar:
            threads = []
    ...
    if found_admin_panels:
        ...
        try:
            ...
            if hosting_location:
                ...
                result.append(f'{G}[+] {Y} Try this potential admin panel link:{W} {potential_admin_panel_link}')
                potential_admin_panel_link = generate_admin_panel_link(domain_ip)
                result.append(f'{G}[+] {Y} Another possible admin panel link:{W} {potential_admin_panel_link}')

        except Exception as e:
            result.append(f'{G}[+] {R} Could not determine hosting location. Error: {e}')
