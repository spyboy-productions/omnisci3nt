...
async def run_scan(target, num_threads, timeout, wordlist, allow_redirects, ssl_verification, dns_servers, file_extensions, total_num_words):
    queue = asyncio.Queue(maxsize=num_threads)

    resolver = aiohttp.AsyncResolver(nameservers=dns_servers.split(', '))
    conn = aiohttp.TCPConnector(limit=num_threads, resolver=resolver, family=socket.AF_INET, verify_ssl=ssl_verification)
    ...
    async with aiohttp.ClientSession(connector=conn, timeout=timeout_config) as session:
        with tqdm(total=total_num_words, desc="Scanning", unit="req") as progress:
            insert_task = asyncio.create_task(insert_url(queue, file_extensions, target, wordlist, allow_redirects))
            workers = [asyncio.create_task(consume_url(queue, session, allow_redirects, progress)) for _ in range(num_threads)]
...
def start_scan(target, num_threads, timeout, wordlist, allow_redirects, ssl_verification, dns_servers, output, data, file_extensions):
    ...
    loop.run_until_complete(
        run_scan(target, num_threads, timeout, wordlist, allow_redirects, ssl_verification, dns_servers, file_extensions, total_num_words)
    )
    print_found_urls()
    loop.close()
